...
class FoundationEstimator:
    ...
    def estimate(
        ...
        materials = {
            ...
            "Excavation (m³)": round(excavation_volume, 2),
            "Hardcore (m³)": round(hardcore_volume, 2),
            "Blinding Concrete (m³)": round(blinding_volume, 2),
            "Foundation Concrete (m³)": round(concrete_volume, 2),
            "Backfill (m³)": round(backfill_volume, 2),
            "DPC (m)": round(dpc_length, 2),
            "Anti-Termite (m²)": round(anti_termite_area, 2),

        }

        labour = {

            "Excavation": round(excavation_volume * 0.35, 2),
            "Concrete Works": round(concrete_volume * 0.50, 2),
            "Backfilling": round(backfill_volume * 0.20, 2),
    ...
    "quantities": {
        ...
        "hardcore": round(hardcore_volume, 2),

        "blinding": round(blinding_volume, 2),

        "concrete": round(concrete_volume, 2),

        "backfill": round(backfill_volume, 2),
